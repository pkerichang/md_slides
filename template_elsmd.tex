...
\definecolor{CustomColor}{RGB}{0,0,33}
...
% color URLs with highlight color
\hypersetup{colorlinks=true, urlcolor=structure}
$endif$

% use system fonts. The sans font will be used
\usefonttheme{professionalfonts}
\setbeamerfont{title}{family=\fontspec{Fira Sans},size=\LARGE}
\setbeamerfont{frametitle}{family=\fontspec{Fira Sans},size=\LARGE}
\setbeamerfont{block title}{size=\Large}
\setbeamerfont{quote}{shape=\upshape}
\setbeamerfont{navigation symbols}{size=\scriptsize}
...
% or optionally replace with 3 / 25 little number
$if(slide-numbers)$
$if(beamer-notes)$
% for the speaker-notes version of the slide, we need a kludge to get the right number to appear
\setbeamertemplate{navigation symbols}{%
  \setcounter{slidenum}{\insertpagenumber / 2 + 1}
  \setcounter{slidetotal}{\insertdocumentendpage / 2}
\theslidenum\ / \theslidetotal}
$else$
...
\usepackage{graphicx}
% enables including SVG files
\usepackage{svg}
% make inkscape render fonts instead, so WYSIWYG
\svgsetup{inkscapelatex=false}
% set figures path
\graphicspath{{./tmp/figures}}
\svgpath{{./tmp/figures}}
\makeatletter
...
% include slide number on beamer note page BUT we have to deal with the fact that all the page counters are thrown off by the addition of notes pages
\setbeamertemplate{note page}{%
  \setcounter{slidenum}{\insertpagenumber / 2}
  \setcounter{slidetotal}{\insertdocumentendpage / 2}
  \strut\hspace*{\fill}\theslidenum\ / \theslidetotal%
  \par%
  \insertnote
}
